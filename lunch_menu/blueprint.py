...
async def before_server_start(app: Sanic):
    cache_url = app.config.get("CACHE_URL", "disk://?shards=0")
    expiration = app.config.get("CACHE_EXPIRATION", "10m")
    soft_expiration = app.config.get("CACHE_SOFT_EXPIRATION", "7m")

    app.ctx.lunch_menu_service = LunchMenuService(cache_url = cache_url, expiration = expiration, soft_expiration = soft_expiration)
